...
### レイヤー素材と切り抜き（重要な実務知見）
- 制作は**レイヤー方式**：`bg.png`（猫なしの世界）＋ `cat.png`（透過の猫スプライト）。
- ⚠️ **Nano Banana の「透過」は市松模様を描いた不透明画像**（実体は alpha 全面255）。そのままでは合成で市松が出る。→ **rembg で切り抜いて本物の透過**にする（`reelcast.media.cutout.remove_background`、要 `pip install -e ".[cutout]"`）。
- 将来は visuals 工程（Gemini）出力に対し、この切り抜きを自動適用する。

### 動き（モーション）
- **フルアニメは作らない**（過剰投資・週5hと矛盾）。**2.5Dレイヤー手続き**で付与：
  - 猫：寝息＝縦サイン伸縮、頭振り＝微小上下（`reelcast.media.animate`）
  - 背景：パララックスのドリフト
  - すべてループ長で整数周期＝シームレス。短尺ループを生成→ffmpegで全長へ無劣化ループ＋音源結合。
  - 配置・動き量は `video.metadata["compose"]`（cat_scale/cat_cx/cat_cy/breathing_amp/bob_px/parallax_px…）で調整。
- **控えめテンポ連動**（librosaでBPM→背景ドリフト速度に緩く反映）は次の増分。
