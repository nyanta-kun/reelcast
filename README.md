...
NEW ─ideation→ IDEATED ─music→ MUSIC_READY ─visuals→ VISUALS_READY
   ─compose→ COMPOSED ─thumbnail→ THUMBNAILED ─shorts→ SHORTS_READY ─metadata→ AWAITING_REVIEW
   ──[人間承認]──> APPROVED ─publish→ PUBLISHED
```

オフライン（アカウント不要）で動く工程：music（取り込み検証）・compose・thumbnail・shorts。
外部API依存：ideation/visuals/metadata（生成AI）・publish（YouTube API）。
